import pygame , random , time , sys , os, json # this is for the save file btw
...
class generateworld:
    def save_game(self):
        ...
        with open(GAMESAVE_FILE, "w") as f:
            json.dump(data, f, indent=4)
        print("Game saved:", data)

    def loading_the_game(self):
        if not os.path.exists(GAMESAVE_FILE):
            print ("No save file found mate.")
            return
        with open(GAMESAVE_FILE, "r") as f:
            data = json.load(f)

            if self.player:
                self.player.rect.x = data.get("x", self.player.rect.x)
                self.player.rect.y = data.get("y", self.player.rect.y)
