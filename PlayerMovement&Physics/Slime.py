...
try:
    enemy_sheet_img = pygame.image.load(os.path.join(script_dir,'Sprite_Img/slime_sprite.png')).convert_alpha()
except pygame.error:
...
class Slime(pygame.sprite.Sprite):
    def __init__(self, SCREEN_WIDTH, y, sprite_sheet, scale):
        ...
        for animation in range(animation_steps):
            image = sprite_sheet.get_image(animation, 16, 16, 2, (0, 0, 0))
...
def main():
    ...
    while run:
        ...
        if len(slime_group) == 0:
            slime = Slime(SCREEN_WIDTH, SCREEN_HEIGHT - 100, enemy_sheet, 2)
            slime_group.add(slime)

        # Update enemy (fixed parameters - added scroll)
        slime_group.update(scroll, SCREEN_WIDTH, player)
