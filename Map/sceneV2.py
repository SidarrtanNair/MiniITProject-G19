...
class Enemy(pygame.sprite.Sprite):
    ...
    def find_ground(self):
        ...
        for block in self.blocks:
            if (block["type"] not in ["bush", "tree_stump", "tree_log", "tree_top"] and 
                abs(block["rect"].centerx - self.rect.centerx) < self.block_width * 2):
                if block["rect"].top > self.rect.bottom:
                    self.ground_y = min(self.ground_y, block["rect"].top)
